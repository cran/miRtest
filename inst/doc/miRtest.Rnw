...
% TITLE PAGE
\title{miRtest v. 1.2 Package Vignette}
...
High-throughput measurements of gene expression are gaining popularity. So are microRNA analyses. The `miRtest' package \cite[] {detection_artmann_submitted} intends to help researchers find differentially expressed miRNAs between two groups.
...
In \cite{detection_artmann_submitted}, we analysed the neurogenesis data from \cite{nielsen09}, where rat Early Neuronal Progenitors from embryonic day 11 and day 13 were compared in miRNA and mRNA microarrays.\\
